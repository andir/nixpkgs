...
{ crateName,
  ...
  in ''
    ...
    # When tests are enabled build all the files in the `tests` directory as test binaries
    if [ -d tests ]; then
      find tests \( -type f -o -type l \) -a -name '*.rs' -print0 | while IFS= read -r -d ''' file; do
        mkdir -p target/bin
